"""Test max calibration model on board"""
import numpy as np, cv2, time, os, glob
from hobot_dnn import pyeasy_dnn as dnn

MODEL = "/home/sunrise/yolo_deploy/yolov8n_2cls_nchw_cpuconv_bayese_640x640_nv12.bin"
...
def bgr_to_nv12(img):
    h,w=img.shape[:2]; scale=min(INP/h,INP/w); nh,nw=int(h*scale),int(w*scale)
    resized=cv2.resize(img,(nw,nh)); canvas=np.full((INP,INP,3),114,np.uint8)
    pt,pl=(INP-nh)//2,(INP-nw)//2; canvas[pt:pt+nh,pl:pl+nw]=resized
    ...
    return np.concatenate([y,uv],axis=0),scale,pl,pt
...
models=dnn.load(MODEL); model=models[0] if isinstance(models,list) else models

img_paths=sorted(glob.glob(os.path.join(VAL_IMG,"*.jpg")))
tp=fp=fn=0; total_t=0
for i,ip in enumerate(img_paths):
    bn=os.path.splitext(os.path.basename(ip))[0]; lp=os.path.join(VAL_LAB,bn+".txt")
    img=cv2.imread(ip); oh,ow=img.shape[:2]; gb,gc=load_gt(lp,ow,oh)
    t0=time.time(); nv12,sc,pl,pt=bgr_to_nv12(img)
    outputs=model.forward(nv12.flatten()); t1=time.time()
    pb,ps,pc=postprocess(outputs,ow,oh,sc,pl,pt); t2=time.time()
    t,f,fn0=match(pb,ps,pc,gb,gc); tp+=t; fp+=f; fn+=fn0; total_t+=(t2-t0)*1000
    if (i+1)%20==0: print(f"{i+1}/{len(img_paths)} gt={len(gb)} pred={len(pb)} tp={t}")

n=len(img_paths); avg_t=total_t/n
p=tp/(tp+fp+1e-6); r=tp/(tp+fn+1e-6); F1=2*p*r/(p+r+1e-6)
print(f"\nMax calibration: {avg_t:.0f}ms F1={F1:.4f} TP={tp} FP={fp} FN={fn}")
